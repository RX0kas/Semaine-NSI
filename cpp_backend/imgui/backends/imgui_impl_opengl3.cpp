...
//  2025-02-18: OpenGL: Lazily reinitialize embedded GL loader for when calling backends from e.g. other DLL boundaries. (#8406)
...
// If you happen to be developing a new feature for this backends (imgui_impl_opengl3.cpp):
...
// Changes to this backends using new APIs should be accompanied by a regenerated stripped loader version.
#define IMGL3W_IMPL
...
bool    ImGui_ImplOpenGL3_Init(const char* glsl_version)
{
    ...
    IMGUI_CHECKVERSION();
    IM_ASSERT(io.BackendRendererUserData == nullptr && "Already initialized a renderer backends!");
...
void    ImGui_ImplOpenGL3_Shutdown()
{
    ...
    IM_ASSERT(bd != nullptr && "No renderer backends to shutdown, or already shutdown?");
    ImGuiIO& io = ImGui::GetIO();
    ImGuiPlatformIO& platform_io = ImGui::GetPlatformIO();
...
{
    ImGui_ImplOpenGL3_Data* bd = ImGui_ImplOpenGL3_GetBackendData();
    IM_ASSERT(bd != nullptr && "Context or backends not initialized! Did you call ImGui_ImplOpenGL3_Init()?");
...
void ImGui_ImplOpenGL3_UpdateTexture(ImTextureData* tex)
{
    ...
    else if (tex->Status == ImTextureStatus_WantUpdates)
    {
        ...
        // This backends choose to use tex->Updates[] but you can use tex->UpdateRect to upload a single region.
        GLint last_texture;
        GL_CALL(glGetIntegerv(GL_TEXTURE_BINDING_2D, &last_texture));
